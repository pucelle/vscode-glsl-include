...
export class GLSLIncludeExtension {
	...
	loadConfig() {
		...
		let matchGlob = this.config.get('matchGlob', '')
		let notMatchGlob = this.config.get('notMatchGlob', '')

		if (matchGlob) {
			this.matchGlob = new minimatch.Minimatch(matchGlob)
		}

		if (notMatchGlob) {
			this.notMatchGlob = new minimatch.Minimatch(notMatchGlob)
		}
	}
	...
	private async compile(document: vscode.TextDocument) {
		...
		try{
			if (destPath && text) {
				...
				this.showChannelMessage(`"${srcPath}" compiled and saved to "${destPath}"`)
	...
	private generateDestPath(srcPath: string) {
		...
		if (srcDirname && destDirPathRelativeToSrc) {
			...
			if (index > -1) {
				...
				destPath = path.resolve(path.dirname(srcDir), destDirPathRelativeToSrc, relativePath)
			}
		}

		if (destPath) {
			destPath = this.formatWithVariableSubstitutions(destPath, srcPath)
		}
